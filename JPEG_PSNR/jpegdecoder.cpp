...
jpeg_decoder::jpeg_decoder(std::string filename, double cosine_idct_org[8][8]) : zigZagStart(0), zigZagEnd(63), losslessFormat(false) {
    ...
    m_rgb = NULL;
#if DEBUGLEVEL_S7es >20
    cout<<"Copied Version"<<endl;
    dumpDecodedBlockInDecimal(cosine_idct);
    cout<<"Original Version"<<endl;
    dumpDecodedBlockInDecimal(cosine_idct_org);
#endif
    for(int y=0; y<8; y++)
    {
        for(int x=0; x<8; x++)
        {
            cosine_idct[x][y]  =  cosine_idct_org[x][y];
        } // end inner loop
    } // end outer loop
...
vector<vector<unsigned char>> jpeg_decoder::Arr2Vec(unsigned char** x, int jpegImageWidth , int jpegImageHeight)
{
...
cv::Mat jpeg_decoder::vec2mat(vector< vector<unsigned char> > &matrix){
...
Scalar jpeg_decoder::getMSSIM( const Mat& i1, const Mat& i2)
{
...
double jpeg_decoder::getPSNR(const cv::Mat& i1, const cv::Mat& i2, int comp_width, int comp_height)
{
        ...
        for (int x = 0; x < 8; ++x) {
            ...
            switch (comp) {
                case COMPONENT_Y:
                    ...
                    tCoeff_Y_dou[picture_y][realx] = raw_dct_value;
...
int jpeg_decoder::display_jpg_bmp(const std::string output_bmp_filename) const {
    ...
    cv::Mat img = cv::imread(output_bmp_filename, 0);
...
int jpeg_decoder::write_yuv_from_jpg_in_yuv(const std::string output_yuv_filename) {
    ...
    std::string name_file_only = path_with_name.substr(found+1);
    std::cout<<path_with_name<<std::endl;
    oss << path_with_name << "_" << jpegImageWidth << "x" << jpegImageHeight << ".yuv";
    ...
    for(int i = 0; i < jpegImageHeight; ++i) {
        for(int j = 0; j < jpegImageWidth; ++j) {
            int idx = j + i * jpegImageWidth;
            temp[idx] = m_YPicture_buffer[i][j];
//            temp[idx] = tCoeff_Y[i][j];
            fwrite( &temp[idx], sizeof(uint_8), 1, pFile );
        }
    }
    
    
#if DEBUGLEVEL_S7es > 50
    std::cout<<"numberOfComponents: "<<components.size()<<std::endl;
    cv::Mat img(jpegImageHeight, jpegImageWidth, CV_8U, temp, cv::Mat::AUTO_STEP);
    imshow("m_YPicture_buffer "+output_yuv_filename, img);
    cv::waitKey(0);
#endif
    delete [] temp;
    ...
    if (components.size() > 1)
    {
        ...
        uint8_t* temp1 = new uint8_t[jpegImageWidth*jpegImageHeight];
        
        for(int i = 0; i < jpegImageHeight; ++i) {
            for(int j = 0; j < jpegImageWidth; ++j) {
                int idx = j + i * jpegImageWidth;
                temp1[idx] = m_CbPicture_buffer[i][j];
                fwrite( &temp1[idx], sizeof(uint_8), 1, pFile );
            }
        }
#if DEBUGLEVEL_S7es > 50
        cv::Mat img1(jpegImageHeight, jpegImageWidth, CV_8U, temp1, cv::Mat::AUTO_STEP);
        imshow("m_CbPicture_buffer "+output_yuv_filename, img1);
        cv::waitKey(0);
#endif
        delete [] temp1;
        uint8_t* temp2 = new uint8_t[jpegImageWidth*jpegImageHeight];
        
        for(int i = 0; i < jpegImageHeight; ++i) {
            for(int j = 0; j < jpegImageWidth; ++j) {
                int idx = j + i * jpegImageWidth;
                temp2[idx] =  m_CrPicture_buffer[i][j];
                fwrite( &temp2[idx], sizeof(uint_8), 1, pFile );
            }
        }
#if DEBUGLEVEL_S7es > 50
        cv::Mat img2(jpegImageHeight, jpegImageWidth, CV_8U, temp2, cv::Mat::AUTO_STEP);
        imshow("m_CrPicture_buffer "+output_yuv_filename, img2);
        cv::waitKey(0);
#endif
        delete [] temp2;
    }
...
void jpeg_decoder::initPositionsBuffersForPictureBuffer() {   
    ...
    tCoeff_Y_dou.resize(comp_height, vector<int> (comp_width, 0));
    ...
    if(components.size() >= COMPONENT_Cb + 1) {
        ...
        tCoeff_Cb_dou.resize(comp_height, vector<int> (comp_width, 0));
    ...
    if(components.size() >= COMPONENT_Cr + 1) {
        ...
        tCoeff_Cr_dou.resize(comp_height, vector<int> (comp_width, 0));
    }
